...
class City(Country):
    def __init__(self, continent, climate, language, population, public_transportation, crime_activity, dangerous):
        super().__init__(continent, climate, language)
        self.population = population
        self.public_transportation = public_transportation
        self.crime_activity = crime_activity
        self.dangerous = dangerous

Rio_de_Janeiro = City('South America','Tropical', 'Portuguese', '6.8M', True, 'Intense ', True)

print(Rio_de_Janeiro.population)
# -------------------------------------------------------------------------------------- #
...
for i in list_of_numbers:
    m = Number(i)
    if m.divisible_by_n(3):
        list_d.append(i)
    elif m.divisible_by_n(4):
        list_d.append(i)
    else:
        continue
